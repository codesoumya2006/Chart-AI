...
export const FlowNode: React.FC<FlowNodeProps> = ({
  ...
  const createMouseEventFromTouch = (touchEvent: React.TouchEvent, _type: 'mousedown' | 'mousemove' | 'mouseup'): React.MouseEvent => {
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        backgroundColor: theme === 'dark' ? 'rgba(20, 20, 20, 0.95)' : 'rgba(250, 250, 250, 0.95)',
        borderColor: isSelected ? '#3b82f6' : (isLocked ? '#7f1d1d' : node.pinned ? '#f59e0b' : (theme === 'dark' ? '#333' : '#d4d4d8')), 
    ...
    >
      ...
      <div className="p-3 text-sm relative flex flex-col flex-grow h-full overflow-hidden">
        ...
        <div className="flex flex-col gap-2 flex-grow h-full overflow-hidden">
          {/* Text Area */}
          <textarea 
            id={`nodeLabel-${node.id}`}
            name={`nodeLabel-${node.id}`}
            className={`bg-transparent border-none focus:outline-none resize-none flex-grow text-sm font-mono leading-relaxed p-1 min-h-[40px] w-full ${
              theme === 'dark' 
                ? 'text-zinc-300 focus:text-white placeholder-zinc-600' 
                : 'text-zinc-700 focus:text-zinc-900 placeholder-zinc-400'
            }`}
