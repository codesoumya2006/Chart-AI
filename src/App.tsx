...
import { 
  GraduationCap, Settings, Download, Upload, Sparkles, Database, MessageSquare, Wand2, Maximize2, Eye, EyeOff, Moon, Sun
} from 'lucide-react';
...
import { SettingsModal, TopicModal, LoadingOverlay, ConfirmModal, AddGeminiKeyModal } from './components/Modals';
...
import { generateAIContent, generateChatResponse, GeminiAPIError } from './utils/aiService';
import { organizeNodesInViewport } from './utils/layout';
import { onAuthStateChange, getFirebaseError } from './utils/firebase';
import { getGeminiKey, setGeminiKey } from './utils/geminiKeyManager';
...
// Constants
import { GRID_SIZE, DEFAULT_NODE_WIDTH, DEFAULT_NODE_HEIGHT, NODE_TYPES } from './constants';
...
function FlowDo() {
  ...
  if (firebaseError) {
    return (
      <div style={{
        ...
        <p style={{ color: '#888', fontSize: '14px', marginTop: '16px', maxWidth: '600px' }}>
  ...
  const [apiKey, setApiKey] = useState(() => {
    // Load API key from localStorage only (no environment variable fallback)
  ...
  // Attach passive: false event listeners for events that call preventDefault
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    const handleWheel = (e: WheelEvent) => canvasInteractions.handleWheel(e as any);
    const handleTouchStart = (e: TouchEvent) => canvasInteractions.handleTouchStart(e as any);
    const handleTouchMove = (e: TouchEvent) => canvasInteractions.handleTouchMove(e as any);
    const handleTouchEnd = (e: TouchEvent) => canvasInteractions.handleTouchEnd(e as any);
    const handleContextMenu = (e: MouseEvent) => canvasInteractions.handleContextMenu(e as any);
  ...
  }, [canvasInteractions.handleWheel, canvasInteractions.handleTouchStart, canvasInteractions.handleTouchMove, canvasInteractions.handleTouchEnd, canvasInteractions.handleContextMenu, canvasRef]);
  ...
  const handleAIOperation = useCallback(async (operation: string, nodeId: string) => {
    ...
    try {
      ...
      // Check if this is a quota/auth error
      if (e instanceof GeminiAPIError && e.shouldRemoveKey) {
        setApiKey('');
        toastError(`API Key Error: ${e.message}\n\nPlease update your API key in Settings.`);
        setShowAddGeminiKeyModal(true);
      } else {
  ...
  const handleAddGeminiKey = useCallback((key: string) => {
    setApiKey(key);
    setGeminiKey(key);
    setShowAddGeminiKeyModal(false);
  ...
  return (
    <div className="w-full h-screen overflow-hidden bg-white dark:bg-zinc-950 text-zinc-900 dark:text-white font-sans select-none relative">
      <Toaster
        ...
        toastOptions={{
          ...
          success: {
            iconTheme: {
              primary: '#a855f7',
              secondary: theme === 'dark' ? '#f4f4f5' : '#18181b',
            },
          },
          error: {
            iconTheme: {
      ...
      <AddGeminiKeyModal
        isOpen={showAddGeminiKeyModal}
        onClose={() => setShowAddGeminiKeyModal(false)}
        onAddKey={handleAddGeminiKey}
      />
      ...
      <div className="absolute top-4 left-4 z-50 pointer-events-none">
        <h1 className="text-3xl font-bold tracking-tighter text-zinc-900 dark:text-white opacity-80 flex items-center gap-2">
          <GraduationCap className="w-8 h-8 text-indigo-500" /> 
          Chart AI 
          <span className="text-sm font-normal text-zinc-500 dark:text-zinc-500">Learning</span>
        </h1>
        <p className="text-zinc-500 dark:text-zinc-500 text-sm mt-1">Right-click: Add Nodes</p>
      </div>
      ...
      <div className="absolute top-4 right-4 z-50 flex gap-2 pointer-events-auto items-center">
        <div className={`text-xs mr-2 font-mono ${
          saveStatus === 'error' ? 'text-red-500' : 'text-zinc-500 dark:text-zinc-500'
        }`}>
          {saveStatus === 'saving' ? 'Saving...' : (saveStatus === 'error' ? 'Error!' : 'Saved')}
        </div>
        <button 
          onClick={handleFitToView} 
          className="bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 border border-zinc-700 dark:border-zinc-700 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-2"
        ...
        <button 
          ...
          className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 dark:from-purple-600/20 dark:to-pink-600/20 hover:from-purple-600/40 hover:to-pink-600/40 dark:hover:from-purple-600/40 dark:hover:to-pink-600/40 text-purple-600 dark:text-purple-300 border border-purple-500/50 dark:border-purple-500/50 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-2"
        ...
        <button 
          ...
          className={`${
            showChatSidebar 
              ? 'bg-indigo-600/40 dark:bg-indigo-600/40 hover:bg-indigo-600/60 dark:hover:bg-indigo-600/60' 
              : 'bg-indigo-600/20 dark:bg-indigo-600/20 hover:bg-indigo-600/40 dark:hover:bg-indigo-600/40'
          } text-indigo-600 dark:text-indigo-300 border border-indigo-500/50 dark:border-indigo-500/50 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-2`}
        >
          <MessageSquare size={14} /> Chat
        </button>
        <button 
          onClick={() => setShowTopicModal(true)} 
          className="bg-purple-600/20 dark:bg-purple-600/20 hover:bg-purple-600/40 dark:hover:bg-purple-600/40 text-purple-600 dark:text-purple-300 border border-purple-500/50 dark:border-purple-500/50 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-2"
        ...
        <button 
          ...
          className={`${
            showMinimap 
              ? 'bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300' 
              : 'bg-zinc-800/50 dark:bg-zinc-800/50 hover:bg-zinc-700/50 dark:hover:bg-zinc-700/50 text-zinc-700 dark:text-zinc-300'
          } border border-zinc-700 dark:border-zinc-700 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-2`}
        ...
        {firebaseUser && (
          <button 
            ...
            className="bg-green-600/20 dark:bg-green-600/20 hover:bg-green-600/40 dark:hover:bg-green-600/40 text-green-600 dark:text-green-300 border border-green-500/50 dark:border-green-500/50 px-3 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center gap-2"
        ...
        <button 
          ...
          className={`${
            firebaseUser 
              ? 'bg-blue-600/20 dark:bg-blue-600/20 hover:bg-blue-600/40 dark:hover:bg-blue-600/40 text-blue-600 dark:text-blue-300 border border-blue-500/50 dark:border-blue-500/50' 
              : 'bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 border border-zinc-700 dark:border-zinc-700'
        ...
        <button 
          ...
          className="p-1.5 bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 rounded-md border border-zinc-700 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300"
        >
          <Settings size={16} />
        </button>
        <button 
          onClick={toggleTheme}
          className="p-1.5 bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 rounded-md border border-zinc-700 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 transition-colors"
        ...
        <button 
          ...
          className="bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 px-3 py-1.5 rounded-md text-xs font-medium border border-zinc-700 dark:border-zinc-700 transition-colors"
        ...
        <button 
          onClick={handleReset} 
          className="bg-zinc-800 dark:bg-zinc-800 hover:bg-zinc-700 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 px-3 py-1.5 rounded-md text-xs font-medium border border-zinc-700 dark:border-zinc-700"
        >
          Reset
        </button>
      </div>
